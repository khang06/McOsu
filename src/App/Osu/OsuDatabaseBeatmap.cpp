...
void OsuDatabaseBeatmapStarCalculator::initAsync()
{
	...
	if (m_iErrorCode == 0)
	{
		...
		double speedNotes = 0.0;
		double speedDifficultStrains = 0.0;
		m_totalStars = OsuDifficultyCalculator::calculateStarDiffForHitObjects(diffres.diffobjects, m_fCS, m_fOD, m_fSpeedMultiplier, m_bRelax, m_bTouchDevice, &aimStars, &aimSliderFactor, &aimDifficultStrains, &speedStars, &speedNotes, &speedDifficultStrains, -1, &m_aimStrains, &m_speedStrains);
		m_aimStars = aimStars;
